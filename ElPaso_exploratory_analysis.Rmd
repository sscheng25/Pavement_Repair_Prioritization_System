...
ggplot(EPCenterline_new, aes(x=crash_count)) + geom_histogram(fill="purple") + scale_x_continuous(limits = c(1, 150))

#including segments with 0 crashes
ggplot(EPCenterline_new, aes(x=crash_count)) + geom_histogram(fill="purple")
```
...
numVars <-
  EPCenterline_final %>%
  dplyr::select(potholes_count, waze_count, crash_count, VMT, total_pop, med_hh_income, med_rent, pct_transport_to_work, pctWhite, pctNotHL, Res_Year) %>%
  st_drop_geometry() %>%
  na.omit()
