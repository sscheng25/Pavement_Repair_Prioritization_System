...
# why my if statement always TRUE???? But when I print it out, there's TURE and FALSE
...
EPCenterline_with_PCI <- EPCenterline_with_PCI %>%
  dplyr::filter(EPCenterline_new$PCI_2018 > 0)

# also use this chunk to clean up the dataframe - remove some of the other columns that we do not need
```
...
## Joining data together (all years)
### potholes
...
ggplot(EPCenterline_new, aes(x=potholes_count)) + geom_histogram(fill="#e6a52f", binwidth = 10) + scale_x_continuous(limits = c(1, 150)) + plotTheme()

#including segments with 0 potholes
ggplot(EPCenterline_new, aes(x=potholes_count)) + geom_histogram(fill="#e6a52f", binwidth = 10) + plotTheme()
...
ggplot(EPCenterline_new, aes(x=crash_count)) + geom_histogram(fill="#e6a52f", binwidth = 10) + 
...
### VMT data - KRISTIN to fix to only include El Paso City tracts
...
#I dont think we actually need to do this join again, we already have everything in here!
#EPCenterline_with_PCI <-
#  EPCenterline_new %>%
...
#EPCenterline_index_PCI_ResYear <-
#  EPCenterline_new3 %>%
#              st_drop_geometry() %>%
#              dplyr::select(index, PCI_2018, Res_Year)
...
### Roadbed data - join to EPCenterline_new here
...
roadbed_base_PCI <- st_join(roadbed_base, EPCenterline_new, join=st_nearest_feature)
...
roadbed_surface_PCI <- st_join(roadbed_surface, EPCenterline_new, join=st_nearest_feature)
...
```
```{r}
# TO DO EARLIER IN THE RMD - let's combine the PRs with their "parent" categories (major PR should be part of major, etc.)
EPCenterline_new_classgrp <- EPCenterline_new %>% st_drop_geometry() %>%
  group_by(CLASS) %>%
  summarise(count=n())
