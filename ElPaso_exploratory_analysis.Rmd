...
#then join back to initial EPCenterline using index as the ID
EPCenterline_with_crash <- merge(EPCenterline, crash_groupings, by = "index", all.x=TRUE)

#replace NAs in crash count column with 0
EPCenterline_with_crash$crash_count[is.na(EPCenterline_with_crash$crash_count)] <- 0
```
...
ggplot() +
  geom_sf(data = El_Paso_city, fill="transparent", color="red") +
  geom_sf(data = EPCenterline_with_crash , aes(color = crash_count)) +
  scale_color_viridis(direction=-1, limits = c(1, 150))+
...
ggplot(EPCenterline_with_crash, aes(x=crash_count)) + geom_histogram(fill="purple") + scale_x_continuous(limits = c(1, 150))

#including segments with 0 crashes
ggplot(EPCenterline_with_crash, aes(x=crash_count)) + geom_histogram(fill="purple")
```
