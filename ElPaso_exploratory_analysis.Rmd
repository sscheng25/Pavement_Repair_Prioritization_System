...
```{r load data sources in github repo, message=FALSE, warning=FALSE}
...
```{r boundary data of El Paso county and city, message=FALSE, warning=FALSE}
texas <-
...
# subset center line data to El Paso city maintained segments
EPCenterline <-
  EPCenterline %>%
...
```{r explore centerline data with PCI values}
...
```{r center line data with MUNR City of El Paso}
ggplot() +
  geom_sf(data = El_Paso_city, color="grey") +
  geom_sf(data = EPCenterline %>%
            subset(MUNR == 'CITY OF EL PASO'), colour="red",) +
  labs(title = "Center line with MUNR CITY OF EL PASO",
       subtitle = "El Paso, TX") + mapTheme()
```
...
```{r pavements with negative PCIs}
# glimpse(center_line)
