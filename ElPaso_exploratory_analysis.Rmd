...
EP_econ <- 
  get_acs(geography = "tract", 
          variables = c("B19013_001E", #median household income
                        ...
                        "B08301_002E", #people with transpo to work and take car/van/truck
                        "B08301_001E"), #total pop - means of transportation estimate
          year = 2019, 
          state = 48,      # 48 for Texas
          geometry = TRUE, 
          county = 141,    # 141 for El Paso county
          output = "wide") %>%
  rename(total_pop_transport =  B08301_001E,
         med_hh_income = B19013_001E,
         med_rent = B25058_001E,
         transport_to_work = B08301_002E) %>%
  dplyr::select("GEOID","NAME","total_pop_transport","med_hh_income","med_rent","transport_to_work","geometry")%>% #drop MOE columns
  mutate(pct_transport_to_work = (ifelse(total_pop_transport > 0, transport_to_work / total_pop_transport,0))*100)
```
...
ggplot()+
  ...
  scale_fill_viridis()+
  labs(title="Percent Population with Car/Van/Truck \nTransportation to Work in 2019",
...
ggplot(pot_holes_sf, aes(y=YEAR)) + 
  geom_bar(width=0.5, color="black", fill = "#FF6666") +
  labs(title = "Year of potholes repairment",
       subtitle = "El Paso, TX") + plotTheme()
