...
EP_age <-
  get_acs(geography = "tract",
          ...
          county = 141,
          output = "wide")%>%
  dplyr::select("GEOID","NAME",all_of(ageVar),"geometry") #need to aggregate &plot - is there a fast way to do this without having to list and add each variable together?, kristin.
...
do.call(grid.arrange,c(mapList, ncol = 3, top = "Race by Census Tract", bottom = "Figure #"))
...
do.call(grid.arrange,c(mapList_econ, ncol = 3, top = "Select Socioeconomics by Census Tract", bottom = "Figure #"))
```
```{r}
...
ggplot()+
  ...
  scale_fill_viridis()+
  labs(title="% with Transportation to Work",
       subtitle="El Paso, TX 2019")
```
