...
```{r echo=TRUE, fig.cap = "Image source: El Paso Times"}
...
### 4.1.2. Features: Road Conditions
...
The histogram below shows most of the traffic jams were heavy or moderate, with a few categorized as light traffic. It is important to keep in mind people do not tend to report light traffic to Waze. 
...
We examined the data related to a road’s anatomy: roadbed surface and base materials. 
...
After engineering our features, we split the 2016-2018 data into a train and test set. We put the train set into several machine learning models and used them to predict PCI. We compared these predictions to the actual PCI scores kept in the test set to evaluate how well each model performs. After selecting a model, we can hypertuned the parameters to further improve its accuracy before applying it to 2019-2021 data. The following sections describe our process for the different models we evaluated and how we ultimately decided to use a Random Forest Regression model.
...
We set up an initial random forest model with mostly default hyperparameters. The initial model returned a MAPE of 26.14%, which is the best model performance so far. We decided to do hyperparameter tuning on the random forest algorithm to achieve even better model accuracy.
...
**Support Vector Machine (SVM)** is another type of supervised machine learning algorithm that is generally applied in classification and regression. Here we built up SVM models with the kernels `polynomial`, `radial`, and `sigmoid`. The best of three models returns MAPE of 47.13%, much lower than the random forest tuned model, so we decide not to work with SVM further.
...
The tuned XGBoost model returns MAPE of 27.18%, similar to the Random Forest model performance.
...
After building and checking the model accuracy for the different regression algorithms, **we decide to go forth with the tuned random forest model**. We carefully examined its model results and error distributions among categorical groups and numeric variables. 

### 5.6.1. Results for Random Forest

```{r RF prediction vs real PCI, message=FALSE, warning=FALSE}
...
The plot below shows the distribution of absolute error by length of each road segment. Errors are generally lower for shorter segments, hovering around 10% absolute error. Around the 500 feet mark, the distribution shows some spikes, indicating greater error variability for segments between 500 and 1000 feet. 

```{r abs error and pavement plot, message=FALSE, warning=FALSE}
...
We looked to the distribution of absolute error across PCI values to see if the model predicts better for higher ratings versus lower, or other patterns that may be present in the data. The plot shows that higher absolute errors occur for road segments that have worse PCI scores - particularly below 25.

```{r abs error and pci plot, message=FALSE, warning=FALSE}
EP_model_rf1 %>%
ggplot(aes(PCI_2018, rf_absError)) +
  geom_bar(position = "dodge", stat = "summary", fun = "mean", fill="#5192B0") +  
  labs(title = "Absolute Error vs. PCI",
...
Zooming in on the raw errors for PCI scores from 25 to 55 (the **very poor** to **poor** range on the qualitative PCI scale), the errors are less than 25, which means that they are likely not changing categories on the qualitative scale. We interpret this further in the costs and benefits section below.
...
```{r echo=TRUE}
knitr::include_graphics("Images/PCI-ratingscale.jpg")

```

## 5.7. Ensemble Modeling with Random Forest

**Ensemble modeling** is a process where multiple models are created to predict an outcome. One of the measures for applying ensemble modeling is using many different sub-models. The ensemble model can then aggregate the prediction of each sub-model and results in one final prediction for the unseen data. The motivation for using ensemble model is to reduce the generalization error of the prediction.

As mentioned before, we classified our features into three categories - **road conditions**, **environment**, and **road network** - so we decided to use the same criterion to build up sub-models.
...
However, the ensemble model does not perform as well as we had hoped. The performance of the ensemble model is worse than our initial random forest model 1. Thus, we decide to continue with our tuned random forest model based on the results in section 5.6.
...
Based on the model training and testing attempts above, we made PCI predictions with *Random Forest Model 1* (`rf_model1`), which is built up in section 5.3.1. In this part, we aimed to get the PCI for 2021 as output of the random forest regression model.
...
## 6.2. Zooming into Specific Areas
...
```{r echo=TRUE}
knitr::include_graphics("Images/PredictionsStreets.jpg")
...
```{r echo=TRUE}
knitr::include_graphics("Images/PredictionsGoogleMaps.jpg")
```

# 7. Application: El Paso Road Viewer

After finalizing our predictive model, we can load the 2021 PCI predictions in our web application, the [El Paso Road Viewer (EPRV)](https://jennaepstein.github.io/El_Paso_Road_Viewer/). This tool will help our client decide where and how to allocate capital improvement funds for road repairs using more than just PCI. The city sent out a community survey where they learned that residents most value equity, safety, traffic congestion, and flood risk when thinking about how to improve their city’s road infrastructure. Using the capital improvement department’s equity hexbin layer as a guide, we created three additional layers to represent each of these values. 

```{r echo=TRUE}
knitr::include_graphics("Images/EPRV_webapp_screenshot.png")
```
