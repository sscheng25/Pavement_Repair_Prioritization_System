...
In this part, we start diving deeper into the existing datasets. To begin with, we apply data cleaning and wrangling on EPCenterline data layer. First, we get rid of some unneeded columns to keep the data frame tidy and readable. We also clip the data layer to the city level to focus more on our study area.
...
To get the X and Y coordinates for waze jam data, we apply some string extraction on the raw waze data and build up a spatial data frame.
...
As is shown in the histogram below, most of the traffic jams collected in waze dataset are heavy and moderate traffic jams, with a few NAs and very few light traffic. This is probably because people do not tend to report light traffic to waze database. The map of waze data points reveals that most of the congestions occur along the main roads in the city. We also do research on the NAs of waze data, and according to our client, that’s probably other categories of waze data which are assigned to the NA group in our project.
...
#NEED TO ALSO CLIP IT WITHIN EL PASO CITY
...
gganimate::animate(potholes_animation, duration=10, renderer = gifski_renderer())
```
...
ggplot() +
  ...
  scale_colour_viridis(option='G',  direction=-1, trans = "log", breaks=potholes_breaks, labels=my_breaks, limits = c(1, 250),
...
ggplot() +
  ...
  scale_colour_viridis(option='G', direction=-1, trans = "log", breaks=potholes_breaks, labels=my_breaks, limits = c(1, 210),
                       name="Number of potholes \nby road length\n")+
...
ggplot(EPCenterline_new, aes(x=potholes_len)) + 
  geom_histogram(fill="#e6a52f", binwidth = 10) + scale_x_continuous(limits = c(1, 150)) +
...
# including segments with 0 potholes
ggplot(EPCenterline_new, aes(x=potholes_len)) + 
  geom_histogram(fill="#e6a52f", binwidth = 10) + 
