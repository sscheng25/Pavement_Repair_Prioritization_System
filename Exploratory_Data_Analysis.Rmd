...
my_breaks = c(1, 5, 10, 25,210)


ggplot() +
  geom_sf(data = EPCenterline_new%>%  filter(potholes_count > 0), aes(color = potholes_count)) +
  scale_colour_viridis(option='G',  trans = "log", breaks=my_breaks, labels=my_breaks, limits = c(1, 250),
...
ggplot() +
  ...
  scale_colour_viridis(option='G', direction=-1, trans = "log", breaks=my_breaks, labels=my_breaks, limits = c(1, 210),
                       name="Number of potholes \nby road length\n")+
...
ggplot() +
  ...
  scale_color_viridis(direction=-1, option='G',limits = c(1, 150))+
  labs(title = "Road Centerlines by Number of Crashes",
       subtitle = "El Paso, TX") + mapTheme()
```
