...
At this point, we also calculate the third nearest neighbors to be used as the engineered features. Here we apply the third nearest neighbor because it can reduce the randomness compared to first and second nearest neighbor and at the same time try to keep most of the spatial feature of the road segment.
...
Looking at the distribution of our features, we decided to explore some transformations. We looking across the distributions of numeric variables to get a sense of how they each look.
...
After examining these distributions, we find that crash length, potholes length, distance to major intersection, and distance to hydrology features are tightly left-skewed. Since one of the asumptions for the linear regression model we'll apply later is that the independent variables should be normally distributed, we decide to apply **log transformation** to the three features mentioned above for better meet the model asumption and help make the relationship more linear.
...
```{r prediction with rf model initial, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
EP_model_rf_initial <- 
  EP_model_test %>%
...
Hyperparameters control how random forest algorithm learns and how it behaves. Unlike the internal parameters that the algorithm automatically optimizes during model training, hyperparameters are model characteristics that we must set in advance. Finding the optimal hyperparameters configuration is challenging and there is no way of knowing the ideal hyperparameters in advance. It is, therefore, that finding an excellent model requires conducting several experiments with different parameters. To avoid the time-consuming manual hyperparameter tuning, we apply the grid-search cross validation here. Grid search uses a grid of predefined hyperparameters to test all possible permutations and return the model variant that leads to the best results. Here, we mainly test on the **mtry** hyperparameter. mtry is the number of variables randomly sampled as candidates at each split, and it's one of the most important hyperparameter for the random forest model that influence the model performance. After running the grid search cv, we get the optimal **mtry = 15**.
...
```{r print the grid search result}
rf_gridsearch
```


### 5.4.3 Tuned Random Forest Model
...
Ensemble modeling is a process where multiple diverse models are created to predict an outcome, one of the measures for applying ensemble modeling is using many different sub-models. The ensemble model can then aggregate the prediction of each sub-model and results in once final prediction for the unseen data. The motivation for using ensemble model is to reduce the generalization error of the prediction.

As mentioned before, we classify our features into three categories, **built environment**, **road connectivity**, and **road conditions**, and we decide to use the same criterion to build up sub-models.

```{r ensemble model 1 -- built environment, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
However, the ensemble model doesn't perform as we expected actually. The performance of the ensemble model is similar to but worse than our initial random forest model 1. Thus, we decide to continue with our initial model.
...
Based on the model training and testing attempts above, we finally decide to make PCI predictions with *Random Forest Model 1*, which is built up in 5.4.1. In this part, we aim to get the PCI for 2021 as output of the random forest regression model.
...
```{r examine na dataframe, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
